...
#ts, ys = RungeKutta4(sistema_n_cuerpos, t_ini, y0_sistema, h, n_pasos)
...
sol = solve_ivp(
    ...
    method='Radau',               # El método (Runge-Kutta-Fehlberg 4(5))
    t_eval=t_eval,               # Puntos donde guardar la solucion
    rtol=1e-9,                   # Tolerancia relativa (Precision)
...
ax.grid(True)
ax.set_title(f"Simulacion N-Cuerpos (Radau) con h={h}, pasos={n_pasos}, tiempo total={n_pasos*h} años terrestres")
